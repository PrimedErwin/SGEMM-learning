...
//gemm func
__global__
void Db_exp_GEMM::matrixMul(const float* A, const float* B, float* C,
	const int M, const int N, const int K)
{
	//basic params
	...
	*reinterpret_cast<float4*>(&matB[_ld_index][bIdx * 4]) = regB[0];
	//matA
	...
	matA[_ld_index][rowA + (colA + 0) * (M_tile + 4)] = regA[0].x;
	matA[_ld_index][rowA + (colA + 1) * (M_tile + 4)] = regA[0].y;
	matA[_ld_index][rowA + (colA + 2) * (M_tile + 4)] = regA[0].z;
	matA[_ld_index][rowA + (colA + 3) * (M_tile + 4)] = regA[0].w;
	__syncthreads();
	...
	for (int i = 0; i < K / K_tile - 1; i++)
	{
		...
		for (int k = 0; k < K_tile; k++)
		{
			...
			regA[0] = *reinterpret_cast<float4*>(&matA[_ld_index][rowC + k * (M_tile + 4)]);
			regA[1] = *reinterpret_cast<float4*>(&matA[_ld_index][rowC + k * (M_tile + 4) + 4]);
			regB[0] = *reinterpret_cast<float4*>(&matB[_ld_index][colC + k * N_tile]);
			regB[1] = *reinterpret_cast<float4*>(&matB[_ld_index][colC + k * N_tile + 4]);
			...
			//k<4, ldgsts32 for each thread, watch for coalesced gmem

			if (k == K_tile - 1)
			{
				*reinterpret_cast<float4*>(&matB[_st_index][bIdx * 4]) = \
					* reinterpret_cast<const float4*>(baseB + rowB * N + colB + (i + 1) * N * K_tile);
				preA = *reinterpret_cast<const float4*>(baseA + rowA * K + colA + (i + 1) * K_tile);
				matA[_st_index][rowA + (colA + 0) * (M_tile + 4)] = preA.x;
				matA[_st_index][rowA + (colA + 1) * (M_tile + 4)] = preA.y;
				matA[_st_index][rowA + (colA + 2) * (M_tile + 4)] = preA.z;
				matA[_st_index][rowA + (colA + 3) * (M_tile + 4)] = preA.w;
				_ld_index ^= 1;
				_st_index ^= 1;
				//__syncthreads();
			}
